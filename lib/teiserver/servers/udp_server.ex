...
defmodule UDPServer do
  ...
  def start_link(_) do
    ...
    {ip, port} =
      {Application.get_env(:udp_server, :ip, {127, 0, 0, 1}),
       Application.get_env(:udp_server, :port, 8202)}

    GenServer.start_link(__MODULE__, [%State{handler: {mod, fun}, ip: ip, port: port}],
      name: __MODULE__
    )
  end
  ...
  def init([%State{} = state]) do
    require Logger
    {:ok, socket} = :gen_udp.open(state.port, [:binary, :inet, {:ip, state.ip}, {:active, true}])
  ...
  def handle_info(
        {:udp, socket, ip, fromport, packet},
        %State{socket: socket, handler: {mod, fun}} = state
      ) do
    IO.puts("UDP info")
    ...
    apply(mod, fun, [
      %Response{ip: format_ip(ip), fromport: fromport, packet: String.trim(packet)}
    ])
