defmodule Teiserver.Telemetry.Tasks.PersistMatchMonthTask do
    ...
    case first_logs do
      [log] ->
        ...
        if log.date.year < today.year or log.date.month < today.month do
          logs = Telemetry.list_match_day_logs(search: [
  ...
  def month_so_far() do
    ...
    Telemetry.list_match_day_logs(search: [
      start_date: Timex.beginning_of_month(now)
    ])
      |> Enum.reduce(@empty_result, fn (log, acc) ->
        extend_segment(acc, log)
  ...
  defp extend_sub_section(existing, data) do
    %{
      aggregate: %{
        ...
        total_duration_seconds: existing.aggregate.total_duration_seconds + data["aggregate"]["total_duration_seconds"],
        weighted_count: existing.aggregate.weighted_count + (data["aggregate"]["weighted_count"] || 0)
      },
  ...
  defp post_process_section(data) do
    mean_duration_seconds = (data.aggregate.total_duration_seconds / data.aggregate.total_count)
      |> round()
