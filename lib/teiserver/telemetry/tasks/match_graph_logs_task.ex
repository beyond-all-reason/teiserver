defmodule Teiserver.Telemetry.MatchGraphLogsTask do
  ...
  def perform(logs, "grouped1", key) do
    converted_logs = logs
    |> Enum.map(fn l ->
      pvp = Map.get(l.data["duel"]["aggregate"], key, 0) + Map.get(l.data["ffa"]["aggregate"], key, 0) + Map.get(l.data["team"]["aggregate"], key, 0)
      pve = Map.get(l.data["raptors"]["aggregate"], key, 0) + Map.get(l.data["scavengers"]["aggregate"], key, 0)
  ...
  def perform(logs, "grouped2", key) do
    ...
    |> Enum.map(fn l ->
      ...
      coop = Map.get(l.data["raptors"]["aggregate"], key, 0) + Map.get(l.data["scavengers"]["aggregate"], key, 0) + Map.get(l.data["bots"]["aggregate"], key, 0)
