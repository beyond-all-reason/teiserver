defmodule Teiserver.Telemetry.Tasks.PersistServerWeekTask do
  ...
  defp extend_segment(existing, %{data: data} = _log) do
    %{
      ...
      tmp_reduction: %{
        unique_users:
          existing.tmp_reduction.unique_users ++ Map.keys(data["minutes_per_user"]["total"]),
        unique_players:
          existing.tmp_reduction.unique_players ++ Map.keys(data["minutes_per_user"]["player"]),
        accounts_created:
          existing.tmp_reduction.accounts_created +
            data["aggregates"]["stats"]["accounts_created"],
        peak_users:
          max(
            existing.tmp_reduction.peak_users,
            data["aggregates"]["stats"]["peak_user_counts"]["total"]
          ),
        peak_players:
          max(
            existing.tmp_reduction.peak_players,
            data["aggregates"]["stats"]["peak_user_counts"]["player"]
          )
      },
      ...
      aggregates: %{
        ...
        minutes: %{
          ...
          spectator:
            existing.aggregates.minutes.spectator + data["aggregates"]["minutes"]["spectator"],
