defmodule Barserver.Telemetry.SimpleClientEventQueries do
  @moduledoc false
  use BarserverWeb, :queries
  alias Barserver.Telemetry.SimpleClientEvent

  # Queries
  @spec query_simple_client_events(list) :: Ecto.Query.t()
  def query_simple_client_events(args) do
  ...
  defp _where(query, :event_type_id, event_type_id) do
    from simple_client_events in query,
      where: simple_client_events.event_type_id == ^event_type_id
  end

  defp _where(query, :event_type_id_in, event_type_ids) do
    from simple_client_events in query,
      where: simple_client_events.event_type_id in ^event_type_ids
  end
  ...
  defp _order_by(query, "Newest first") do
    from simple_client_events in query,
      order_by: [desc: simple_client_events.timestamp]
  end

  defp _order_by(query, "Oldest first") do
    from simple_client_events in query,
      order_by: [asc: simple_client_events.timestamp]
  end
  ...
  defp _preload(query, :users) do
    from simple_client_events in query,
      left_join: users in assoc(simple_client_events, :user),
      preload: [user: users]
  end

  defp _preload(query, :event_types) do
    from simple_client_events in query,
      left_join: event_types in assoc(simple_client_events, :event_type),
      preload: [event_type: event_types]
  end

  @spec get_simple_client_events_summary(list) :: map()
  def get_simple_client_events_summary(args) do
    query =
      from simple_client_events in SimpleClientEvent,
        join: event_types in assoc(simple_client_events, :event_type),
        group_by: event_types.name,
        select: {event_types.name, count(simple_client_events.event_type_id)}
