defmodule Teiserver.Telemetry.ComplexServerEventQueries do
  @moduledoc false
  use TeiserverWeb, :queries
  alias Teiserver.Telemetry.ComplexServerEvent

  # Queries
  @spec query_complex_server_events(list) :: Ecto.Query.t()
  def query_complex_server_events(args) do
  ...
  defp _where(query, :id, id) do
    from complex_server_events in query,
      where: complex_server_events.id == ^id
  end

  defp _where(query, :user_id, userid) do
    from complex_server_events in query,
      where: complex_server_events.user_id == ^userid
  end
  ...
  defp _order_by(query, "Newest first") do
    from complex_server_events in query,
      order_by: [desc: complex_server_events.timestamp]
  end
  ...
  def get_aggregate_detail(event_type_id, key, start_datetime, end_datetime, limit \\ 50) do
    ...
    FROM telemetry_complex_server_events e
