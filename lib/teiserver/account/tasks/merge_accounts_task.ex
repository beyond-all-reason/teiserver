defmodule Teiserver.Account.MergeAccountsTask do
  ...
  def perform(deleting_id, keeping_id) do
    ...
    query = "UPDATE teiserver_telemetry_server_events SET user_id = #{keeping_id} WHERE user_id = #{deleting_id}"
    Ecto.Adapters.SQL.query(Repo, query, [])
    ...
    # Accolades
    query = "UPDATE teiserver_account_accolades SET giver_id = #{keeping_id} WHERE giver_id = #{deleting_id}"
    Ecto.Adapters.SQL.query(Repo, query, [])

    query = "UPDATE teiserver_account_accolades SET recipient_id = #{keeping_id} WHERE recipient_id = #{deleting_id}"
    Ecto.Adapters.SQL.query(Repo, query, [])
