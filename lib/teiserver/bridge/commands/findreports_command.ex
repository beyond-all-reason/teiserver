defmodule Teiserver.Bridge.Commands.FindreportsCommand do
  ...
  def execute(_interaction, options_map) do
    mode = options_map["mode"] || "message_id" # Default to message_id
...
defp get_action_content(action) do
  jump_link = if action.discord_message_id != nil, do: ["**Action Link:**", "- https://discord.com/channels/#{Communication.get_guild_id()}/#{get_channel("other")}/#{action.discord_message_id}"], else: []
  ...
  with {report_id, ""} <- Integer.parse(id_str),
    ...
    report_link =
      if report.discord_message_id == nil or channel == nil do
        ...
        ["**Report Link:**", "- https://discord.com/channels/#{Communication.get_guild_id()}/#{channel}/#{report.discord_message_id}"]
    end
...
defp handle_message_id(id_str) do
  with {message_id, ""} <- Integer.parse(id_str),
       action when not is_nil(action) <- Moderation.get_action(search: [discord_message_id: message_id]) do
    reports =
