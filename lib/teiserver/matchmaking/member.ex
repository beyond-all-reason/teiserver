defmodule Teiserver.Matchmaking.Member do
  ...
  For example %{rating: 23.4, uncertainty: 3.2}
  """
  @type rating :: %{rating: float(), uncertainty: float()}
  ...
  def get_member_rating(player_ids, game_type) do
    ...
    default = %{rating: default.rating_value, uncertainty: default.uncertainty}
    ...
    case Enum.find(Teiserver.Game.get_ratings_for_users(player_ids), &(&1.name == game_type)) do
      ...
      rt ->
        ...
        rating = Enum.sum_by(ratings, & &1.rating_value)
        uncertainty = Enum.sum_by(ratings, & &1.uncertainty)

        if n == 0,
          do: default,
          else: %{rating: rating / n, uncertainty: uncertainty / n}
    end
  end
end
