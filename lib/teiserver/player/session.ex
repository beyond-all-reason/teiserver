defmodule Teiserver.Player.Session do
  ...
  def handle_continue({:snapshot, snapshot}, _state) do
    ...
    with {:ok, state} <- restore_parties(state, snapshot.party) do
      broadcast_user_update!(user, :menu)
  ...
  defp restore_parties(state, party_snapshot) do
    with {:ok, state} <- rejoin_current_party(state, party_snapshot),
         {:ok, state} <- rejoin_invite_parties(state, party_snapshot) do
      {:ok, state}
    end
  end
