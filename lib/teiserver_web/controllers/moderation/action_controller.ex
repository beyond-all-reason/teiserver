defmodule TeiserverWeb.Moderation.ActionController do
  ...
  def show(conn, %{"id" => id}) do
    ...
    |> assign(:guild_id, Communication.get_guild_id())
    |> assign(
      :channel,
      Communication.get_discord_channel("Public moderation log")
    )
  ...
  def create(conn, %{"action" => action_params}) do
    ...
    duration_seconds =
      case DateHelper.human_input_to_datetime(action_params["duration"] || "") do
        {:ok, future_datetime} -> DateTime.diff(future_datetime, DateTime.utc_now())
        nil -> nil
      end
