          ...
          app when not is_nil(app) ->
            case OAuth.create_token(user, %{
              ...
              {:ok, token} ->
                ...
                if token.refresh_token, do: Map.put(credentials, :refresh_token, token.refresh_token.value), else: credentials
                user_data = Map.take(user, ~w(id name email icon colour roles permissions restrictions shadowbanned last_login last_played)a)
                timestamps = %{
              ...
              {:error, %Ecto.Changeset{} = changeset} ->
                conn |> put_status(400) |> json(%{error: "Token creation failed: " <> format_changeset_errors(changeset)})
            end
          nil ->
            conn |> put_status(400) |> json(%{error: "user_admin OAuth application not found. Please run 'mix teiserver.tachyon_setup' to create it."})
        end
