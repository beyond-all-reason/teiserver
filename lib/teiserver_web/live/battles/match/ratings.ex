defmodule TeiserverWeb.Battle.MatchLive.Ratings do
  ...
  def handle_params(params, _url, socket) do
    ...
    rating_type = case Map.get(params, "rating_type") do
      nil -> "Large Team"
  ...
  defp update_match_list(
         %{assigns: %{rating_type: rating_type, filters: filters, current_user: current_user, page: page, limit: limit}} =
           socket
       ) do
  ...
  defp build_url(opts) do
    # Only include limit in query params, not rating_type since it's in the path
    base_params = %{
      "limit" => opts.limit
    }
    ...
    # Use the original URL structure: /battle/ratings for Large Team, /battle/ratings/Name for others
    base_path = if opts.rating_type == "Large Team" do
      "/battle/ratings"
    else
      "/battle/ratings/#{URI.encode_www_form(opts.rating_type)}"
    end
