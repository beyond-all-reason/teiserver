defmodule TeiserverWeb.Account.SettingsLive.Index do
  ...
  def handle_event("open-form", %{"key" => key}, %{assigns: assigns} = socket) do
    new_key = if assigns.selected_key == key do
      nil
    else
      key
    end
  ...
  def handle_event("reset-value", _, %{assigns: %{selected_key: key, current_user: user}} = socket) do
  ...
  def handle_event("set-to", %{"value" => new_value}, %{assigns: %{selected_key: key, current_user: user}} = socket) do
    insert_or_update_config(user.id, key, new_value)
  ...
  defp insert_or_update_config(userid, key, value) do
    case Config.get_user_config(userid, key) do
      nil ->
        {:ok, _config} = Config.create_user_config(%{
          "user_id" => userid,
          "key" => key,
          "value" => "true"
        })
      user_config ->
        {:ok, _config} = Config.update_user_config(user_config, %{
          "value" => value
        })
    end
  end
