defmodule TeiserverWeb.ModerationLive.User.ClearGDPRForget do
  @moduledoc false
  ...
  @impl LiveView
  def handle_event("perform-clear", _params, %Socket{assigns: assigns} = socket) do
    result = UserLib.clear_gdpr_forget(assigns.user, assigns.scope)

    case result do
      {:ok, _any} ->
        socket
        |> put_flash(:info, "GDPR forget cleared")
        |> redirect(to: ~p"/moderation/users/#{assigns.user.id}")
        |> noreply()

      _other ->
        socket
        |> put_flash(:error, "Error clearing GDPR forget flag")
        |> noreply()
    end
  end
end
