defmodule Central.Helpers.GeneralTestLib do
  ...
  def make_account_group_membership(group_id, user_id, data) do
    {:ok, gm} = Account.create_group_membership(%{
      "group_id" => group_id,
      "user_id" => user_id,
  ...
  def seed() do
    ...
    cousin_group = make_account_group("cousin group", parent_group.id, %{}, %{"see_group" => true})
    unrelated_group = make_account_group("unrelated group", nil, %{}, %{"see_group" => true, "self_add_members" => true})
  ...
  def conn_setup(permissions \\ [], flags \\ []) do
    ...
    {user, jwt} = unless :no_user in flags do
      user = make_user(%{
        "name" => "Current user",
    ...
    conn = if :no_login in flags or :no_user in flags do
      # build_conn()
      login(build_conn(), "")
    else
      login(build_conn(), user.email)
