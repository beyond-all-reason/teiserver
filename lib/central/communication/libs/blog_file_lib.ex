defmodule Central.Communication.BlogFileLib do
  ...
  def _search(query, :simple_search, ref) do
    ...
    from blog_files in query,
      where: (
            ilike(blog_files.name, ^ref_like)
        )
  end
  ...
  def store_file(the_file, upload_path, filename) do
    the_file_save_path = Application.get_env(:central, Central.Communication.BlogFile)
    ...
    if moved_stat == upload_stat do
      ...
      {:error, "There was an error uploading the file. Please report the issue to Teifion with the following numbers and a copy of the file you were uploading.  moved_stat: #{moved_stat}, upload_stat: #{upload_stat}"}
    end
  end

  def delete_file(%{file_path: nil}), do: true
  def delete_file(file) do
    base_path = Application.get_env(:central, Central.Communication.BlogFile)
    |> Keyword.get(:save_path)

    file_name = String.replace(file.file_path, base_path, "")
    actual_path = "#{base_path}#{file_name}"

    moved_stat = 'stat --printf="%s" "#{actual_path}"'
    |> :os.cmd()
    |> to_string
    |> String.slice(0..17)
