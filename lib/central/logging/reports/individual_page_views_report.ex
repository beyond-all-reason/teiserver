defmodule Central.Logging.IndividualPageViewsReport do
  ...
  def run(conn, params) do
    ...
    # Date range
    {start_date, end_date} = DatePresets.parse(
      params["date_preset"],
      params["start_date"],
      params["end_date"]
    )

    key = case params["mode"] do
      "User" -> "users.name"
      "Group" -> "groups.name"
      "Path (Full)" -> "(logs.section || '/' || logs.path)"
      "Path (1 part)" -> "split_part((logs.section || '/' || logs.path), '/', 1)"
      "Path (2 parts)" -> "(logs.section || '/' || split_part(logs.path, '/', 1))"
      "Path (3 parts)" -> "(logs.section || '/' || split_part(logs.path, '/', 1)|| '/' || split_part(logs.path, '/', 2))"
      "Path (4 parts)" -> "(logs.section || '/' || split_part(logs.path, '/', 1)|| '/' || split_part(logs.path, '/', 2) || '/' || split_part(logs.path, '/', 3))"
    end

    wheres = [
      (if params["path"] != "", do: "logs.path LIKE '#{params["path"] |> String.replace("'", "''")}'"),
      (if params["section"] != "", do: "logs.section = '#{params["section"] |> String.replace("'", "''")}'"),
      (if params["group"] != "", do: "ugms.group_id = '#{params["group"] |> String.replace("'", "''")}'")
    ]
    ...
    joins = [
      (if params["mode"] == "Group", do: "JOIN account_groups groups ON groups.id = users.admin_group_id"),
      (if params["group"] != "", do: "JOIN account_group_memberships ugms ON ugms.user_id = logs.user_id")
    ]
    |> Enum.filter(fn r -> r != nil end)
    |> Enum.join("    ")
