defmodule Central.Logging.MostRecentUsersReport do
  ...
  def run(conn, params) do
    ...
    query = from logs in PageViewLog,
      ...
      group_by: [users.name, users.icon, users.colour, users.id, users.email, groups.name, groups.icon, groups.colour],
      order_by: [desc: max(logs.inserted_at)],
      select: {
        users.id, users.name, users.email, users.icon, users.colour,
        groups.name, groups.icon, groups.colour,
