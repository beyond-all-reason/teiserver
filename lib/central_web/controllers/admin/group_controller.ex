defmodule CentralWeb.Admin.GroupController do
  ...
  def index(conn, params) do
    memberships = case allow?(conn, "admin.group.show") do
      true -> nil
  ...
  def new(conn, _params) do
    types = GroupTypeLib.get_all_group_types
  ...
  def create(conn, %{"group" => group_params}) do
    group_type = if group_params["type_id"] != "" do
  ...
  def delete_membership(conn, %{"user_id" => user_id, "group_id" => group_id}) do
    ...
    access_allowed = (
      (user_id |> String.to_integer == conn.user_id)
      or (group_access[:admin])
      # or (user_id != conn.user_id and group_access[:invite_members] and GroupLib.access?(conn, group.id))
    )
