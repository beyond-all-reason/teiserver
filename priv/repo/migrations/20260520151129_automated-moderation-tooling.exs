defmodule Teiserver.Repo.Migrations.AutomatedModerationTooling do
  ...
  def change do
    create_if_not_exists table(:banned_ips) do
      add :cidr, :text

      timestamps()
    end

    create_if_not_exists unique_index(:banned_ips, [:cidr])

    create_if_not_exists table(:banned_domains) do
      add :domain, :text

      timestamps()
    end

    create_if_not_exists unique_index(:banned_domains, [:domain])
    ...
    create_if_not_exists table(:banned_phrases) do
