defmodule Teiserver.OAuth.TokenTest do
  ...
  test "cannot use expired refresh token", %{user: user, app: app} do
    way_back_then = Timex.shift(Timex.now(), days: -90)
    assert {:ok, token} = OAuth.create_token(user, app, now: way_back_then)
    assert {:error, :expired} = OAuth.refresh_token(token.refresh_token)
  end
