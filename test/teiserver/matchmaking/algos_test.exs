defmodule Teiserver.Matchmaking.AlgosTest do
  ...
  describe "ignore OS" do
    ...
    test "not enough to fill one team" do
      members = [mk_member(1)]
      assert Algos.ignore_os(members, 1, 2) == :no_match
    end

    test "one team" do
      members = [m1, m2] = [mk_member(1), mk_member(2)]
      assert {:match, [team]} = Algos.ignore_os(members, 1, 2)
      assert MapSet.new(team) == MapSet.new([[m1], [m2]])
    end

    test "two teams" do
      members = Enum.map(1..4, &mk_member/1)
      assert {:match, [_team1, _team2]} = Algos.ignore_os(members, 1, 2)
    end

    test "more teams" do
      members = Enum.map(1..4, &mk_member/1)
      assert {:match, [team]} = Algos.ignore_os(members, 1, 3)
      assert Enum.count(team) == 3
    end

    test "bigger team" do
      members = Enum.map(1..3, &mk_member/1)
      assert :no_match = Algos.ignore_os(members, 2, 2)
    end

    test "bigger team with match" do
      members = Enum.map(1..4, &mk_member/1)
      assert {:match, [_team]} = Algos.ignore_os(members, 2, 2)
    end

    test "bigger team with party" do
      members = [mk_member([1, 2]), mk_member(3), mk_member(4)]
      assert {:match, [team]} = Algos.ignore_os(members, 2, 2)
    ...
    test "ordering doesn't matter" do
      ...
      assert {:match, _} = Algos.ignore_os(members, 3, 2)
    end
  end
