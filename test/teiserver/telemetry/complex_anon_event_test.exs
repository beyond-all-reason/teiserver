defmodule Teiserver.Telemetry.ComplexAnonEventTest do
  ...
  test "complex anon events" do
    ...
    # Log the event
    {result, _} =
      Telemetry.log_complex_anon_event(hash, "anon.complex_user_event-#{r}", %{"key1" => "value1", "key2" => "value2"})

    assert result == :ok

    assert Telemetry.list_complex_anon_events() |> Enum.count() == 1
    assert Telemetry.list_complex_anon_events(search: [hash: hash]) |> Enum.count() == 1
