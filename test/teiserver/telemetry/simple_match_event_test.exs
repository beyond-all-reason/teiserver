defmodule Teiserver.Telemetry.SimpleMatchEventTest do
  ...
  test "match events" do
    ...
    query = "DELETE FROM telemetry_simple_simple_match_events;"
    Ecto.Adapters.SQL.query(Repo, query, [])

    user = TeiserverTestLib.new_user("simple_simple_match_event_user")
    ...
    # Log the event
    {result, _} =
      Telemetry.log_simple_match_event(match.id, user.id, "match.user_event", 13)

    assert result == :ok

    assert Telemetry.list_simple_match_events() |> Enum.count() == 1
    assert Telemetry.list_simple_match_events(search: [user_id: user.id]) |> Enum.count() == 1
