defmodule Teiserver.Telemetry.SimpleLobbyEventTest do
  ...
  test "simple lobby events" do
    ...
    assert Telemetry.list_simple_lobby_events(search: [user_id: user.id]) |> Enum.count() == 1
    assert Telemetry.list_simple_lobby_events(search: [lobby_id: lobby.id]) |> Enum.count() == 1
