defmodule Teiserver.Telemetry.ServerEventTest do
  ...
  test "server events" do
    ...
    # Log with no user
    {result, _} = Telemetry.log_server_event(nil, "server.no_user_event", %{key1: "value1", key2: "value2"})
    assert result == :ok
    ...
    # Log with a user
    {result, _} = Telemetry.log_server_event(user.id, "server.no_user_event", %{key1: "value1", key2: "value2"})
    assert result == :ok
