defmodule Teiserver.Protocols.V1.TachyonConfigTest do
  ...
  test "game configs", %{user: user, socket: socket} do
    _tachyon_send(socket, %{cmd: "c.config.game_get", keys: [
      "key1",
      "key2"
    ]})
    ...
    # Now set some
    _tachyon_send(socket, %{cmd: "c.config.game_set", configs: %{
      "key1" => "Some string",
      "key2" => [1,2,3]
    }})
    reply = _tachyon_recv(socket)
  ...
  test "user configs", %{user: user, socket: socket} do
    ...
    _tachyon_send(socket, %{cmd: "c.config.user_get", keys: [
      # Keys they're allowed
      "general.Colour scheme", "teiserver.Show flag",
    ...
    # Set values
    _tachyon_send(socket, %{cmd: "c.config.user_set", configs: %{
