defmodule Teiserver.Protocols.Spring.SpringPartyTest do
  ...
  describe "invite" do
    ...
    test "invalid player", %{socket1: sock1} do
      party_id = create_party!(sock1)
      invite_to_party(sock1, party_id, "lolnope-this-is-not-a-username-for-party")
  ...
  describe "handle invite" do
    ...
    test "must be in party", ctx do
      invite_to_party(ctx.socket2, ctx.party_id, ctx.user1.name)
  ...
  defp invite_to_party(socket, party_id, username) do
    msg_id = :rand.uniform(1_000_000) |> to_string()
    _send_raw(socket, "##{msg_id} c.party.invite_to_party #{party_id} #{username}\n")
  end

  defp invite_to_party!(socket, party_id, username) do
    invite_to_party(socket, party_id, username)
