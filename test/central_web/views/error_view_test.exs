defmodule CentralWeb.ErrorViewTest do
  ...
  test "renders 500 graceful", %{conn: conn} do
    {:safe, result} = CentralWeb.ErrorView.render("500_graceful.html", [conn: conn, msg: "msg", info: "info"])
    assert Enum.join(result, "") =~ "Something isn't quite right"
  end

  test "renders 500 handled", %{conn: conn} do
    {:safe, result} = CentralWeb.ErrorView.render("500_handled.html", [conn: conn, msg: "msg", info: "info"])
