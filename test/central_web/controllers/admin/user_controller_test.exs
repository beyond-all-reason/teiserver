defmodule CentralWeb.Admin.UserControllerTest do
  ...
  describe "create user" do
    test "redirects to show when data is valid", %{conn: conn, child_group: child_group} do
      conn = post(conn, Routes.admin_user_path(conn, :create), user: Map.put(@create_attrs, :admin_group_id, child_group.id))
  ...
  describe "edit user" do
    ...
    test "renders form for editing secured user", %{conn: conn, parent_group: parent_group} do
      user2 = GeneralTestLib.make_user(%{"name" => "secured-user", "admin_group_id" => "#{parent_group.id}"})
  ...
  describe "permissions" do
    ...
    test "edit_permissions secured resource", %{conn: conn, parent_group: parent_group} do
      user2 = GeneralTestLib.make_user(%{"email" => "user2@user2", "admin_group_id" => "#{parent_group.id}"})
    ...
    test "update permissions", %{conn: conn, main_group: main_group} do
      user = GeneralTestLib.make_user(%{"email" => "user2@user2", "admin_group_id" => "#{main_group.id}"})

      resp = post conn, Routes.admin_user_path(conn, :update_permissions, user), permissions: %{
        "\"admin.user\"" => ["admin.user.show", "admin.user.create",
     "admin.user.update", "admin.user.delete", "admin.user.report"],
      }
    ...
    test "update permissions from source", %{conn: conn, main_group: main_group} do
      target_user = GeneralTestLib.make_user(%{"email" => "user2@user2-target", "admin_group_id" => "#{main_group.id}"})
      source_user = GeneralTestLib.make_user(%{"email" => "user2@user2-source", "admin_group_id" => "#{main_group.id}", "permissions" => ["admin.user.show", "admin.user.create", "admin.user.update", "admin.user.delete", "admin.user.report"]})

      resp = post conn, Routes.admin_user_path(conn, :update_permissions, target_user), account_user: source_user.id

      assert resp.private[:phoenix_flash]["success"] == "User permissions updated successfully."
      assert redirected_to(resp) == Routes.admin_user_path(resp, :show, target_user) <> "#permissions"
    end

    test "wipe", %{conn: conn, main_group: main_group} do
      user = GeneralTestLib.make_user(%{"email" => "user2@user2", "admin_group_id" => "#{main_group.id}", "permissions" => ["xyz"]})
    ...
    test "update_permissions secured resource", %{conn: conn, parent_group: parent_group} do
      ...
      resp = post conn, Routes.admin_user_path(conn, :update_permissions, user2), permissions: %{}
