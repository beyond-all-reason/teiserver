defmodule CentralWeb.Admin.GroupControllerTest do
  ...
  @valid_attrs %{colour: "some content", icon: "some content", name: "some name", data: %{}, see_group: false, see_members: false, invite_members: true, self_add_members: true, type_id: ""}
  ...
  describe "listing" do
    ...
    test "search - get all", %{conn: conn} do
      conn = post conn, Routes.admin_group_path(conn, :search), search: %{
        "name" => "%%%",
        "active" => "All",
        "order" => "Name (Z-A)",
      }
  ...
  describe "showing" do
    ...
    test "shows chosen resource with a type", %{conn: conn, main_group: main_group} do
      ...
      group = GeneralTestLib.make_account_group("showing - a type", main_group.id, %{"Test field name" => "321321showing"}, %{"group_type" => "adm gct - update"})
      conn = get conn, Routes.admin_group_path(conn, :show, group)
      assert html_response(conn, 200) =~ "Name"
  ...
  describe "creation" do
    ...
    test "creates resource when an empty type is selected", %{conn: conn} do
      conn = post conn, Routes.admin_group_path(conn, :create), group: Map.put(@valid_attrs, "type_id", "")
      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.admin_group_path(conn, :show, id)
      assert conn.private[:phoenix_flash]["info"] == "User group created successfully."
    ...
    test "creates resource when a type is selected", %{conn: conn} do
      ...
      conn = post conn, Routes.admin_group_path(conn, :create), group: Map.merge(@valid_attrs, %{
  ...
  describe "editing" do
    ...
    test "editing when it has a type", %{conn: conn, main_group: main_group} do
      ...
      group = GeneralTestLib.make_account_group("editing - a type", main_group.id, %{"Test field name" => "887345created"}, %{"group_type" => "adm gct - editing"})
    ...
    test "redirects when trying to edit a group you can't access", %{conn: conn, parent_group: group} do
      conn = get conn, Routes.admin_group_path(conn, :edit, group)
      assert redirected_to(conn) == Routes.admin_group_path(conn, :index)
      assert conn.private[:phoenix_flash]["danger"] == "Unable to find that group"
    end
    ...
    test "redirects when trying to edit a group you can see but not edit", %{conn: conn} do
      group = GeneralTestLib.make_account_group("edit - see access", nil, %{}, %{
        "see_group" => true
      })
      conn = get conn, Routes.admin_group_path(conn, :edit, group)
  ...
  describe "update" do
    test "updates chosen resource and redirects when data is valid", %{conn: conn, main_group: group} do
    ...
    test "updates chosen resource when it has a type", %{conn: conn, main_group: main_group} do
      create_group_type("adm gct - update")
      group = GeneralTestLib.make_account_group("update - a type", main_group.id, %{"0" => "321321created"}, %{"group_type" => "adm gct - update"})

      conn = put conn, Routes.admin_group_path(conn, :update, group), group: Map.merge(@valid_attrs, %{
    ...
    test "does not update chosen resource and renders errors when data is invalid", %{conn: conn, main_group: group} do
  ...
  describe "membership" do
    test "add/update/remove membership", %{conn: conn, main_group: main_group} do
      ...
      # Add membership
      conn = post conn, Routes.admin_group_path(conn, :create_membership), group_id: main_group.id, account_user: "##{child_user.id}"
      ...
      # Update
      conn = get conn, Routes.admin_group_path(conn, :update_membership, main_group.id, child_user.id), role: "admin"

      assert conn.private[:phoenix_flash]["info"] == "User membership updated successfully."
      assert redirected_to(conn) == Routes.admin_group_path(conn, :show, main_group) <> "#members"

      # Now remove
      conn = get conn, Routes.admin_group_path(conn, :delete_membership, main_group.id, child_user.id)
    ...
    test "add with no access", %{conn: conn} do
      ...
      conn = post(conn, Routes.admin_group_path(conn, :create_membership), group_id: group.id, account_user: "##{child_user.id}")
