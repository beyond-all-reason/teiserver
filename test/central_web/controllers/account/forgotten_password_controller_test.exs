defmodule CentralWeb.Account.ForgottenPasswordControllerTest do
  ...
  describe "mark password as forgotten" do
    test "render form", %{conn: conn} do
      ...
      assert html_response(conn, 200) =~ "Please enter the email address of your account here. A link to reset your password will be sent to the address."
    end
  end
  ...
  describe "submit request" do
    test "email2 - not empty", %{conn: conn} do
      ...
      conn = post(conn, Routes.account_session_path(conn, :send_password_reset), email: dummy.email, email2: dummy.email)
    ...
    test "just email", %{conn: conn} do
      dummy = GeneralTestLib.make_user()
      conn = post(conn, Routes.account_session_path(conn, :send_password_reset), email: dummy.email, email2: "")
    ...
    test "bad key-value pair", %{conn: conn} do
      ...
      conn = post(conn, Routes.account_session_path(conn, :send_password_reset), email: dummy.email, email2: "", key: key, value: "x")
      assert html_response(conn, 200) =~ "Password reset request"
      assert conn.private[:phoenix_flash]["success"] == nil
      assert conn.private[:phoenix_flash]["warning"] == nil
  ...
  describe "reset password form" do
    test "no link", %{conn: conn} do
      conn = get(conn, Routes.account_session_path(conn, :password_reset_form, "--non-valid-code--"))
      assert redirected_to(conn) == "/"
  ...
  describe "reset password post" do
    test "nil code", %{conn: conn} do
      conn = post(conn, Routes.account_session_path(conn, :password_reset_post, "--no-code-value--"), pass1: "", pass2: "")
      assert redirected_to(conn) == "/"
      assert conn.private[:phoenix_flash]["warning"] == "Unable to find link"
    end
    ...
    test "bad purpose", %{conn: conn} do
      ...
      conn = post(conn, Routes.account_session_path(conn, :password_reset_post, code.value), pass1: "", pass2: "")
      assert redirected_to(conn) == "/"
      assert conn.private[:phoenix_flash]["warning"] == "Link cannot be found"
    end
    ...
    test "passwords don't line up", %{conn: conn} do
      ...
      conn = post(conn, Routes.account_session_path(conn, :password_reset_post, code.value), pass1: "valid", pass2: "noooope")
      assert html_response(conn, 200) =~ "Password reset form"
    ...
    test "correct", %{conn: conn} do
      ...
      conn = post(conn, Routes.account_session_path(conn, :password_reset_post, code.value), pass1: "password1", pass2: "password1")
      assert redirected_to(conn) == "/"
