defmodule CentralWeb.Account.RegistrationNewUserControllerTest do
  ...
  @valid_attrs %{name: "new user", email: "new_user@new_user.com", password: "new_password", password_confirmation: "new_password"}
  ...
  describe "create user" do
    ...
    test "inlvalid attrs - short password", %{conn: conn} do
      conn = post(conn, Routes.account_registration_path(conn, :create), user: Map.merge(@valid_attrs, %{password: "1234", password_confirmation: "1234"}))
      assert html_response(conn, 200) =~ "Oops, something went wrong!"
      new_user = Account.get_user([search: [name: "new user"]])
      assert new_user == nil
    end
    ...
    test "inlvalid attrs - mismatched password", %{conn: conn} do
      conn = post(conn, Routes.account_registration_path(conn, :create), user: Map.put(@valid_attrs, :password_confirmation, "long long password"))
      assert html_response(conn, 200) =~ "Oops, something went wrong!"
      new_user = Account.get_user([search: [name: "new user"]])
      assert new_user == nil
    end
