defmodule CentralWeb.Account.GroupControllerTest do
  ...
  describe "show" do
    ...
    test "show with a type", %{conn: conn, main_group: main_group} do
      ...
      group = GeneralTestLib.make_account_group("showing - a type", main_group.id, %{"Test field name" => "321321showing"}, %{"group_type" => "acc gct - show"})
    ...
    test "show without membership and is private", %{conn: conn, parent_group: parent_group} do
      ...
      assert conn.private[:phoenix_flash]["warning"] == "Unable to access this group"
  ...
  describe "edit" do
    ...
    test "edit with membership and type", %{conn: conn, main_group: main_group} do
      ...
      group = GeneralTestLib.make_account_group("editing - a type", main_group.id, %{"Test field name" => "321321showing"}, %{"group_type" => "acc gct - edit"})
    ...
    test "edit without membership but is public", %{conn: conn} do
      ...
      assert conn.private[:phoenix_flash]["warning"] == "You do not have edit access to that group"
  ...
  describe "update" do
    test "update with membership", %{conn: conn, main_group: main_group} do
      conn = put(conn, Routes.account_group_path(conn, :update, main_group.id), group: @update_attrs)
    ...
    test "update with membership and type", %{conn: conn, main_group: main_group} do
      ...
      group = GeneralTestLib.make_account_group("editing - a type", main_group.id, %{"Test field name" => "443355created"}, %{"group_type" => "acc gct - update"})
      conn = put(conn, Routes.account_group_path(conn, :update, group.id), group: Map.merge(@update_attrs, %{
    ...
    test "update with invalid data", %{conn: conn, main_group: main_group} do
      conn = put(conn, Routes.account_group_path(conn, :update, main_group.id), group: %{"name" => ""})
    ...
    test "update without membership and is private", %{conn: conn, parent_group: parent_group} do
      conn = put(conn, Routes.account_group_path(conn, :update, parent_group.id), group: @update_attrs)
      assert conn.private[:phoenix_flash]["warning"] == "Unable to access this group"
      assert redirected_to(conn) == Routes.account_group_path(conn, :index)
    end
    ...
    test "update without membership but is public", %{conn: conn} do
      unrelated_group = Account.get_group!(search: [name: "unrelated group"])
      conn = put(conn, Routes.account_group_path(conn, :update, unrelated_group.id), group: @update_attrs)
      assert conn.private[:phoenix_flash]["warning"] == "You do not have edit access to that group"
      assert redirected_to(conn) == Routes.account_group_path(conn, :show, unrelated_group.id)
    end
  end
  ...
  describe "create membership" do
    test "create without existing membership - allowed bad data", %{conn: conn, main_group: main_group} do
      conn = post(conn, Routes.account_group_path(conn, :create_membership), %{"account_user" => "", "group_id" => main_group.id})
      assert redirected_to(conn) == Routes.account_group_path(conn, :show, main_group.id) <> "#members"
      assert conn.private[:phoenix_flash]["warning"] == "User was unable to be added to group."
    end

    test "create without existing membership - allowed", %{conn: conn, user: user} do
      unrelated_group = Account.get_group!(search: [name: "unrelated group"])
      conn = post(conn, Routes.account_group_path(conn, :create_membership), %{"account_user" => "##{user.id}", "group_id" => unrelated_group.id})
      assert redirected_to(conn) == Routes.account_group_path(conn, :show, unrelated_group.id) <> "#members"
      assert conn.private[:phoenix_flash]["success"] == "User added to group."
    end

    test "create without existing membership - not allowed", %{conn: conn, user: user, parent_group: parent_group} do
      conn = post(conn, Routes.account_group_path(conn, :create_membership), %{"account_user" => "##{user.id}", "group_id" => parent_group.id})
      assert redirected_to(conn) == Routes.account_group_path(conn, :show, parent_group.id) <> "#members"
      assert conn.private[:phoenix_flash]["danger"] == "You do not have the access to add that user to this group."
    end
  end
  ...
  describe "membership" do
    test "add/update/remove membership", %{conn: conn, main_group: main_group} do
      ...
      # Add membership
      conn = post conn, Routes.account_group_path(conn, :create_membership), group_id: main_group.id, account_user: "##{child_user.id}"

      assert conn.private[:phoenix_flash]["success"] == "User added to group."
      assert redirected_to(conn) == Routes.account_group_path(conn, :show, main_group) <> "#members"

      # Update
      conn = put conn, Routes.account_group_path(conn, :update_membership, main_group.id, child_user.id), role: "admin"

      assert conn.private[:phoenix_flash]["info"] == "User membership updated successfully."
      assert redirected_to(conn) == Routes.account_group_path(conn, :show, main_group) <> "#members"

      # Now remove
      conn = delete conn, Routes.account_group_path(conn, :delete_membership, main_group.id, child_user.id)
    ...
    test "add with no access", %{conn: conn} do
      ...
      conn = post(conn, Routes.account_group_path(conn, :create_membership), group_id: group.id, account_user: "##{child_user.id}")
    ...
    test "remove with no access", %{conn: conn} do
      ...
      conn = delete(conn, Routes.account_group_path(conn, :delete_membership, group, child_user.id))
      assert redirected_to(conn) == Routes.account_group_path(conn, :show, group) <> "#members"
      assert conn.private[:phoenix_flash]["danger"] == "You do not have the access to remove that user from this group."
    end
  end
end
