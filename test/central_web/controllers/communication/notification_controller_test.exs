defmodule CentralWeb.Communication.NotificationControllerTest do
  ...
  test "create test notififcation", %{conn: conn, user: user} do
    ...
    conn = post conn, Routes.communication_notification_path(conn, :quick_new), f: %{
      "user_id" => "##{user.id}",
      "title" => "title",
      "body" => "body",
      "icon" => "icon",
  ...
  test "delete all", %{conn: conn, user: user} do
    ...
    Communication.notify(user.id, %{title: "title", body: "body", icon: "icon", colour: "colour", redirect: "redirect"}, 3, false)
    Communication.notify(user.id, %{title: "title", body: "body", icon: "icon", colour: "colour", redirect: "redirect"}, -5, false)
